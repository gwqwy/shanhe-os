...
__saho_require("E:/文件/代码系统/stones/算术库/main.saho", function () {
  let 最大公约数;
  let 最小公倍数;
  let 约分;
  let 是否质数;
  let 质因数分解;
  let 取整数值;
  最大公约数 = __saho_defn(function 最大公约数(a, b) {
    let 余;
    if (__saho_truthy(__saho_lt(a, 0))) {
      a = __saho_neg(a);
    }
    if (__saho_truthy(__saho_lt(b, 0))) {
      b = __saho_neg(b);
    }
    while (__saho_truthy(!__saho_eq(b, 0))) {
      余 = __saho_mod(a, b);
      a = b;
      b = 余;
    }
  ...
  最小公倍数 = __saho_defn(function 最小公倍数(a, b) {
    let 公;
    if (__saho_truthy((__saho_truthy(__saho_eq(a, 0)) || __saho_truthy(__saho_eq(b, 0))))) {
      return 0;
    }
    公 = 最大公约数(a, b);
    return __saho_div(__saho_mul(a, b), 公);
    return null;
  }, ["a", "b"], "最小公倍数");
  约分 = __saho_defn(function 约分(分子, 分母) {
    let 符号;
    let 公;
    if (__saho_truthy(__saho_eq(分母, 0))) {
      __saho_throw(`分母不能是 0。`);
    }
    符号 = 1;
    if (__saho_truthy(__saho_lt(分母, 0))) {
      符号 = __saho_neg(符号);
      分母 = __saho_neg(分母);
    }
    if (__saho_truthy(__saho_lt(分子, 0))) {
      符号 = __saho_neg(符号);
      分子 = __saho_neg(分子);
    }
    公 = 最大公约数(分子, 分母);
    if (__saho_truthy(__saho_eq(公, 0))) {
      return [0, 分母];
    }
    return [__saho_mul(符号, __saho_div(分子, 公)), __saho_div(分母, 公)];
    return null;
  }, ["分子", "分母"], "约分");
  是否质数 = __saho_defn(function 是否质数(n) {
    let 除数;
    if (__saho_truthy(!__saho_eq(n, 取整数值(n)))) {
      return false;
    }
    if (__saho_truthy(__saho_lt(n, 2))) {
      return false;
    }
    if (__saho_truthy(__saho_lt(n, 4))) {
      return true;
    }
    if (__saho_truthy(__saho_eq(__saho_mod(n, 2), 0))) {
      return false;
    }
    除数 = 3;
    while (__saho_truthy(__saho_le(__saho_mul(除数, 除数), n))) {
      if (__saho_truthy(__saho_eq(__saho_mod(n, 除数), 0))) {
        return false;
      }
      除数 = __saho_add(除数, 2);
    }
  ...
  质因数分解 = __saho_defn(function 质因数分解(n) {
    let 因子;
    let 除数;
    因子 = [];
    if (__saho_truthy(__saho_lt(n, 2))) {
    ...
    while (__saho_truthy(__saho_eq(__saho_mod(n, 2), 0))) {
      (__saho_get(因子, "添加"))(2);
      n = __saho_div(n, 2);
    }
    除数 = 3;
    while (__saho_truthy(__saho_le(__saho_mul(除数, 除数), n))) {
      while (__saho_truthy(__saho_eq(__saho_mod(n, 除数), 0))) {
        (__saho_get(因子, "添加"))(除数);
        n = __saho_div(n, 除数);
      }
      除数 = __saho_add(除数, 2);
    }
    if (__saho_truthy(__saho_gt(n, 1))) {
      (__saho_get(因子, "添加"))(n);
    }
    return 因子;
    return null;
  }, ["n"], "质因数分解");
  取整数值 = __saho_defn(function 取整数值(x) {
    let 段;
    段 = __saho_split(__saho_str(x), `.`);
    if (__saho_truthy(__saho_eq(__saho_len(段), 1))) {
      return x;
    }
    return __saho_tonum(__saho_index(段, 0));
    return null;
  }, ["x"], "取整数值");
  return new Map([["最大公约数", 最大公约数], ["最小公倍数", 最小公倍数], ["约分", 约分], ["是否质数", 是否质数], ["质因数分解", 质因数分解], ["取整数值", 取整数值]]);
});
__saho_require("E:/文件/代码系统/stones/文本库/main.saho", function () {
  let 是否回文;
  let 居中;
  let 左对齐;
  let 统计字符;
  let 分行;
  let 连接行;
  是否回文 = __saho_defn(function 是否回文(文) {
    return __saho_eq(文, __saho_reversed(文));
    return null;
  }, ["文"], "是否回文");
  居中 = __saho_defn(function 居中(文, 宽度, 填充) {
    let 差;
    let 左;
    let 右;
    let 侧;
    差 = __saho_sub(宽度, __saho_len(文));
    if (__saho_truthy(__saho_le(差, 0))) {
      return 文;
    }
    左 = __saho_div(差, 2);
    右 = __saho_sub(差, 左);
    侧 = __saho_mul(填充, 左);
    return __saho_add(__saho_add(侧, 文), __saho_mul(填充, 右));
    return null;
  }, ["文", "宽度", "填充"], "居中");
  左对齐 = __saho_defn(function 左对齐(文, 宽度, 填充) {
    let 差;
    差 = __saho_sub(宽度, __saho_len(文));
    if (__saho_truthy(__saho_le(差, 0))) {
      return 文;
    }
    return __saho_add(文, __saho_mul(填充, 差));
    return null;
  }, ["文", "宽度", "填充"], "左对齐");
  统计字符 = __saho_defn(function 统计字符(文) {
    let 计数;
    let 字;
    计数 = new Map([]);
    for (字 of __saho_iter(文)) {
      if (__saho_truthy(__saho_contains(计数, 字))) {
        __saho_set(计数, 字, __saho_add(__saho_index(计数, 字), 1));
      } else {
        __saho_set(计数, 字, 1);
      }
    }
  ...
  分行 = __saho_defn(function 分行(文) {
    return __saho_split(文, `
`);
    return null;
  }, ["文"], "分行");
  连接行 = __saho_defn(function 连接行(行列表) {
    return __saho_join(行列表, {"分隔": `
`});
    return null;
  }, ["行列表"], "连接行");
  return new Map([["是否回文", 是否回文], ["居中", 居中], ["左对齐", 左对齐], ["统计字符", 统计字符], ["分行", 分行], ["连接行", 连接行]]);
});
__saho_require("E:/文件/代码系统/stones/游戏库/main.saho", function () {
  let 掷骰子;
  let 洗牌;
  let 抽奖;
  let 抛硬币;
  掷骰子 = __saho_defn(function 掷骰子(面数) {
    return __saho_std_random["数"](1, 面数);
    return null;
  }, ["面数"], "掷骰子");
  洗牌 = __saho_defn(function 洗牌(原列表) {
    let 结果;
    let 末;
    let 位置;
    let 暂;
    结果 = __saho_tolist(原列表);
    末 = __saho_sub(__saho_len(结果), 1);
    while (__saho_truthy(__saho_gt(末, 0))) {
      位置 = __saho_std_random["数"](0, 末);
      暂 = __saho_index(结果, 末);
      __saho_set(结果, 末, __saho_index(结果, 位置));
      __saho_set(结果, 位置, 暂);
      末 = __saho_sub(末, 1);
    }
    return 结果;
  ...
  抽奖 = __saho_defn(function 抽奖(名单, 个数) {
    let 池;
    if (__saho_truthy(__saho_gt(个数, __saho_len(名单)))) {
      __saho_throw(`名单只有 ${__saho_str(__saho_len(名单))} 个人，抽不出 ${__saho_str(个数)} 个。`);
    }
    池 = 洗牌(名单);
    return __saho_slice(池, 0, 个数);
    return null;
  }, ["名单", "个数"], "抽奖");
  抛硬币 = __saho_defn(function 抛硬币() {
    if (__saho_truthy(__saho_eq(__saho_std_random["数"](0, 1), 0))) {
      return `正`;
    }
    return `反`;
    return null;
  }, [], "抛硬币");
  return new Map([["掷骰子", 掷骰子], ["洗牌", 洗牌], ["抽奖", 抽奖], ["抛硬币", 抛硬币]]);
});
__saho_require("E:/文件/代码系统/stones/中文数字/main.saho", function () {
  let 数位;
  let 单位;
  let 到中文;
  let 到数;
  ...
  单位 = [``, `十`, `百`, `千`];
  到中文 = __saho_defn(function 到中文(n) {
    let 结果;
    let 位;
    let 个;
    let 段;
    if (__saho_truthy(__saho_lt(n, 0))) {
      return __saho_add(`负`, 到中文(__saho_neg(n)));
    }
    if (__saho_truthy(__saho_gt(n, 9999))) {
      __saho_throw(`中文数字包目前支持 0 到 9999。`);
    }
    if (__saho_truthy(__saho_eq(n, 0))) {
      return `零`;
    }
    结果 = ``;
    位 = 0;
    while (__saho_truthy(__saho_gt(n, 0))) {
      个 = __saho_mod(n, 10);
      if (__saho_truthy(__saho_eq(个, 0))) {
        if (__saho_truthy((__saho_truthy(!__saho_eq(结果, ``)) && __saho_truthy(!__saho_eq(__saho_slice(结果, 0, 1), `零`))))) {
          结果 = __saho_add(`零`, 结果);
        }
      } else {
        段 = __saho_add(__saho_index(数位, 个), __saho_index(单位, 位));
        if (__saho_truthy((__saho_truthy((__saho_truthy(__saho_gt(位, 0)) && __saho_truthy(__saho_eq(个, 1)))) && __saho_truthy(__saho_eq(位, 1))))) {
          段 = `十`;
        }
        结果 = __saho_add(段, 结果);
      }
      n = __saho_div(__saho_sub(n, __saho_mod(n, 10)), 10);
      位 = __saho_add(位, 1);
    }
    return 结果;
    return null;
  }, ["n"], "到中文");
  到数 = __saho_defn(function 到数(中文) {
    let 负;
    let 数字;
    let 单位值;
    let 总;
    ...
    let 当前;
    let 字;
    if (__saho_truthy(__saho_eq(中文, `零`))) {
      return 0;
    }
    负 = false;
    if (__saho_truthy(__saho_eq(__saho_slice(中文, 0, 1), `负`))) {
      负 = true;
      中文 = __saho_slice(中文, 1);
    }
    数字 = new Map([[__saho_key(`一`), 1], [__saho_key(`二`), 2], [__saho_key(`三`), 3], [__saho_key(`四`), 4], [__saho_key(`五`), 5], [__saho_key(`六`), 6], [__saho_key(`七`), 7], [__saho_key(`八`), 8], [__saho_key(`九`), 9]]);
    单位值 = new Map([[__saho_key(`十`), 10], [__saho_key(`百`), 100], [__saho_key(`千`), 1000]]);
    总 = 0;
    段 = 0;
    当前 = 0;
    for (字 of __saho_iter(中文)) {
      if (__saho_truthy(__saho_contains(数字, 字))) {
        当前 = __saho_index(数字, 字);
      } else if (__saho_truthy(__saho_eq(字, `零`))) {
        当前 = 0;
      } else if (__saho_truthy(__saho_contains(单位值, 字))) {
        if (__saho_truthy(__saho_eq(当前, 0))) {
          当前 = 1;
        }
        段 = __saho_add(段, __saho_mul(当前, __saho_index(单位值, 字)));
        当前 = 0;
      } else {
        __saho_throw(__saho_add(`认不出中文数字里的字：`, 字));
      }
    }
    总 = __saho_add(__saho_add(总, 段), 当前);
    if (__saho_truthy(负)) {
      总 = __saho_neg(总);
    }
    return 总;
  ...
  return new Map([["数位", 数位], ["单位", 单位], ["到中文", 到中文], ["到数", 到数]]);
});
...
(function () {
  ...
  let 游戏库;
  let 中文数字;
  算术库 = __saho_require("E:/文件/代码系统/stones/算术库/main.saho");
  文本库 = __saho_require("E:/文件/代码系统/stones/文本库/main.saho");
  游戏库 = __saho_require("E:/文件/代码系统/stones/游戏库/main.saho");
  中文数字 = __saho_require("E:/文件/代码系统/stones/中文数字/main.saho");
  __saho_print(`公约数: ${__saho_str((__saho_get(算术库, "最大公约数"))(12, 18))}`);
  __saho_print(`约分: ${__saho_str((__saho_get(算术库, "约分"))(36, 48))}`);
  __saho_print(`质因数分解: ${__saho_str((__saho_get(算术库, "质因数分解"))(360))}`);
  __saho_print(`是否质数: ${__saho_str((__saho_get(算术库, "是否质数"))(97))}`);
  __saho_print((__saho_get(文本库, "居中"))(`卅`, 11, `-`));
  __saho_print(`回文: ${__saho_str((__saho_get(文本库, "是否回文"))(`上海自来水来自海上`))}`);
  __saho_print(`字符统计: ${__saho_str((__saho_get(文本库, "统计字符"))(`abracadabra`))}`);
  __saho_std_random["种子"](3);
  __saho_print(`掷骰子: ${__saho_str((__saho_get(游戏库, "掷骰子"))(6))}`);
  __saho_print(`抛硬币: ${__saho_str((__saho_get(游戏库, "抛硬币"))())}`);
  __saho_print(`抽奖: ${__saho_str((__saho_get(游戏库, "抽奖"))([`小明`, `小红`, `小刚`, `小丽`], 2))}`);
  __saho_print(`95 = ${__saho_str((__saho_get(中文数字, "到中文"))(95))}`);
  __saho_print(`1024 = ${__saho_str((__saho_get(中文数字, "到中文"))(1024))}`);
  __saho_print(`回转: ${__saho_str((__saho_get(中文数字, "到数"))((__saho_get(中文数字, "到中文"))(2026)))}`);
})();
